...
var flagNC = false; //WebSocketで追加されたコメントを選択したらtrueにする
//var gon
...
var rname = null;
var cc = 0; //WebSocketで追加されたコメントのカウンタ
var cid;
var comme = new Array; //WebSocketで追加されたコメントのデータを格納する配列
comme[0] = null;
var lastX = null; //受信時に受信側が閲覧しているコメントの座標
var lastY = null;
...
function Return3(pid, user){

  rname = user;
  $('#new_comment [id=comment_p_id]').val(pid);
  $('#state').html("<b>" + user + "さんに対する返信</b>");
  $('#new_comment').show();
  $('#submitbutton').hide();
  $('#submitbutton2').show();
  $('#newcommentbutton').show();
//window.alert(document.form1.comment_p_id.value +", "+ pid+", "+ r_pid);
  } 
...
//  window.alert(document.form1.comment_article_id.value + document.form1.comment_page.value + document.form1.comment_p_id.value);
  ...
  ws.send("COMM#WS#" + r_x + "#WS#" + r_y + "#WS#" + document.form1.comment_comment.value + "#WS#" + document.form1.comment_p_id.value + "#WS#" + document.form1.comment_user_id.value + "#WS#" + gon.hoge + "#WS#" + pdfpage + "#WS#"  + JSON.stringify(data));
...
//              var listcom =document.getElementById('pdf_' + eval(i+3)).innerHTML;
	  ...
	  if (comme[0] != null){  //WebSocketで新規追加されたコメをクリックした時にflagBを立てる
	    for (var i = 0; i < cc; i=i+1){
	      var pos2_x = Number(comme[i][0]);
	      var pos2_y = Number(comme[i][1]);
	      var pos2_page = Number(comme[i][6]);
	      if (x > pos2_x && x < pos2_x + 50 && y > pos2_y && y < pos2_y + 50 && pos2_page == pdfpage){
	        flagB = true;
	        flagNC = true;
	        r_x = pos2_x;
	        r_y = pos2_y;
...
　             }
          ...
          else {  //WebSocketで追加されたコメ・既存コメをクリック時
            lastX = com_x;
            lastY = com_y;
	    ...
	    $('#state').html("");
            ...
            $("#reload").html("");
            if (comme[0] != null){
              for(var i = 0; i < cc; i = i+1){  //追加コメの数だけループ
                if (comme[i][0] == com_x && comme[i][1] == com_y &&comme[i][5] == gon.hoge && comme[i][6] == pdfpage){  //既存コメクリック時
	              ...
	              if (comme[i][3] == com2.getAttribute("comid")){
	                ...
	                $("#res").append("for <i>" + com2.getAttribute("uid") + "</i>　from <i>" + comme[i][4] + "</i>：<input onclick=\"Return2("+ comme[i][8] + ", '" + comme [i][4] + "')\" type='button' value='返信'><br />　 " + comme[i][2] + "<br>");
	                break;
                      }
                    }
                  }
                  if (comme[i][3] != "" && comme[i][7]){  //返信コメの表示
          	    $("#reload").html("<a href=javascript:location.reload()>以下のコメントが追加されています。表示を整えるには更新してください。</a><br>" );
	            $("#res").append("for <i>" + comme[i][7] + "</i>　from <i>" + comme[i][4] + "</i>：<input onclick=\"Return2("+ comme[i][8] + ", '" + comme [i][4] + "')\" type='button' value='返信'><br />　 " + comme[i][2] + "<br>");
                 }
                  if ( comme[i][3] == "" ){  //新規コメの表示
                    $("#reload").html("<a href=javascript:location.reload()>以下のコメントが追加されています。表示を整えるには更新してください。</a><br>" );
                    $("#res").append("from <i>" + comme[i][4] + "</i>：<input onclick=\"Return2("+ comme[i][8] + ", '" + comme [i][4] + "')\" type='button' value='返信'><br />　 " + comme[i][2] + "<br>");
                  }
                }
                else if (x > r_x && x < r_x + 50 && y > r_y && y < r_y + 50){  //WebSocketで追加されたコメをクリック時
                  lastX = r_x;
                  lastY = r_y;
                  if (comme[i][0] == r_x && comme[i][1] == r_y &&comme[i][5] == gon.hoge && comme[i][6] == pdfpage){
                    if (comme[i][3] != "" && comme[i][7]){  //返信コメの表示
          	      $("#reload").html("<a href=javascript:location.reload()>以下のコメントが追加されています。表示を整えるには更新してください。</a><br>" );
	              $("#res").append("for <i>" + comme[i][7] + "</i>　from <i>" + comme[i][4] + "</i>：<input onclick=\"Return3("+comme[i][8]+", '" + comme [i][4] + "')\" type='button' value='返信'><br />　 " + comme[i][2] + "<br>");
                    }
                    if ( comme[i][3] == "" ){  //新規コメの表示
                      $("#reload").html("<a href=javascript:location.reload()>以下のコメントが追加されています。表示を整えるには更新してください。</a><br>" );
                      $("#res").append("from <i>" + comme[i][4] + "</i>：<input onclick=\"Return3("+comme[i][8]+", '" + comme [i][4] + "')\" type='button' value='返信'><br />　 " + comme[i][2] + "<br>");
                    }
                  }
                }
                else {
...
//-------------------------------------------------------------------    
    ...
    // 受信したデータの解析
    var data = JSON.parse(event.data);    //送られてきたデータを配列dataに格納
    if (data.cid){                           //data.cidが存在すればそれをcidに代入
      cid = data.cid;
//      window.alert(cid);
    }
    ...
    if (data.comflag == 1){
//      window.alert(lastX+", "+data.x);
      data.comment = data.comment.replace(/\r?\n/g, "<br />　 ");
      if (data.x == lastX && data.y == lastY && data.article == gon.hoge && data.page == pdfpage){ //以下、コメント受信時に受信者がそのコメントと同じ座標にある既存コメントのダイアログを開いていた場合に、受信したコメントをダイアログに表示させるための記述
        if (data.pid != "" && data.rname == null){  //既存コメントに対する返信を受信した場合
          var flag1 = 0;
          for (var i = 0; i >= 0; i= i+1){
            var com1 = document.getElementById("pdf_"+i);
	    if (data.pid == com1.getAttribute("comid")){
              $("#reload").html("<a href=javascript:location.reload()>以下のコメントが追加されています。表示を整えるには更新してください。</a><br>" );
	      $("#res").append("for <i>" + com1.getAttribute("uid") + "</i>　from <i>" + data.user + "</i>：<input onclick=\"Return2("+ cid + ", '" + data.user + "')\" type='button' value='返信'><br />　 " + data.comment + "<br>");
//	      $("#res").append("<a href='/comments/"+data.cid+"/edit'>編集</a>" );
              flag1 =1;
	      break;
            }
          }
          if (cc > 0 && flag1 == 0){
            for (var j = 0; j < cc; j++){
              if (data.x == comme[j][0] && data.y == comme[j][1]){
//                window.alert(0);
                $("#reload").html("<a href=javascript:location.reload()>以下のコメントが追加されています。表示を整えるには更新してください。</a><br>" );
	        $("#res").append("for <i>" + comme[j][4] + "</i>　from <i>" + data.user + "</i>：<input onclick=\"Return2("+ cid + ", '" + data.user + "')\" type='button' value='返信'><br />　 " + data.comment + "<br>");
	        data.rname = data.user;
              }
            }
          }
        }
	else if (data.pid != "" && data.rname != null){  //WebSocketで追加されたコメントに対する返信を受信した場合
          $("#reload").html("<a href=javascript:location.reload()>以下のコメントが追加されています。表示を整えるには更新してください。</a><br>" );
	  $("#res").append("for <i>" + data.rname + "</i>　from <i>" + data.user + "</i>：<input onclick=\"Return2("+ cid + ", '" + data.user + "')\" type='button' value='返信'><br />　 " + data.comment + "<br>");
        }
        else if ( data.pid == "" ){  //新規コメントを受信した場合
          $("#reload").html("<a href=javascript:location.reload()>以下のコメントが追加されています。表示を整えるには更新してください。</a><br>" );
          $("#res").append("from <i>" + data.user + "</i>：<input onclick=\"Return2("+ cid + ", '" + data.user + "')\" type='button' value='返信'><br />　 " + data.comment + "<br>"); 
        }
      }
      if (data.article == gon.hoge && data.page == pdfpage && data.flagB == false){  //吹き出しアイコンをポップさせる記述
        var image = new Image();
        image.src = "/assets/sample1.png";
